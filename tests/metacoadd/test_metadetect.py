"""
Most tests are adapted from metadetect repo: https://github.com/esheldon/metadetect/blob/master/metadetect/tests/test_metadetect.py
The others were generated by ChatGPT 5.6 Sol.
"""
...
from metacoadd.fitting import get_gauss_psf_runner
...
def _check_propagated_masks(
    ...
    """Check scientific results and propagated object mask bits."""
    _check_result_array(
        result,
        model,
        nband,
    ...
    for shear_type in SHEAR_TYPES:
        catalog = result[shear_type]

        assert "bmask" in catalog.dtype.names
        assert "ormask" in catalog.dtype.names

        assert np.all((catalog["bmask"] & bmask_bits) == bmask_bits)
        assert np.all((catalog["ormask"] & ormask_bits) == ormask_bits)
...
def test_detect():
    ...
    image, _, weight, bmask, ormask = md.get_coadd_multiband(
        mbobs,
...
@pytest.mark.parametrize("model", SUPPORTED_MODELS)
def test_metadetect_mask_propagation_all(model):
    """Test to mask propagation."""
    rng = np.random.RandomState(seed=53341)
    mbobs = Sim(rng).get_mbobs()

    bmask_bits = MDET_CONFIG["nodet_flags"]
    ormask_bits = 2**4
...
@pytest.mark.parametrize("model", SUPPORTED_MODELS)
def test_metadetect_mask_propagation_some(model):
    """Test to mask propagation."""
    rng = np.random.RandomState(seed=53341)
    mbobs = Sim(rng).get_mbobs()
    ...
    # Only one band contains these bits. The coadd should preserve
    # them through the pixel-by-pixel bitwise OR.
    for obs in mbobs[1]:
        obs.set_bmask(
            np.full(
                obs.image.shape,
...
def test_get_psf_stats():
    """Test that the combined PSF size is self-consistent."""
    nband = 3

    # This simulation generates a different PSF in each band,
    # making the weighted-average check non-trivial.
    mbobs = make_mbobs_sim(
        seed=10,
        nband=nband,
    )
    ...
    # Use an independent RNG with the same seed as the driver.
    # This reproduces the same sequence of PSF-fit guesses.
    psf_runner = get_gauss_psf_runner(np.random.RandomState(seed=11))
    ...
    for obslist in mbobs:
        for obs in obslist:
            psf_result = psf_runner.go(obs.psf.copy())

            assert psf_result["flags"] == 0
            assert np.isfinite(psf_result["T"])
            assert psf_result["T"] > 0

            psf_sizes.append(psf_result["T"])
            psf_weights.append(np.median(obs.weight[obs.weight != 0]))

    psf_sizes = np.asarray(psf_sizes)
    psf_weights = np.asarray(psf_weights)

    expected_T = np.sum(psf_weights * psf_sizes) / np.sum(psf_weights)

    md = _make_driver(
        config,
        np.random.RandomState(seed=11),
    )
    measured_T = md.get_T_psf(mbobs)

    assert np.isfinite(measured_T)
    assert measured_T > 0

    # A positive weighted average must lie inside the range of
    # the individual measurements.
    assert measured_T >= np.min(psf_sizes)
    assert measured_T <= np.max(psf_sizes)
