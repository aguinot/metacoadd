...
def _make_data(rng, shear, wcs, has_pixel=True, psf_fwhm=0.9):
    ...
    im_noise = rng.normal(scale=noise, size=im.shape)
    ...
    jacobian = ngmix.Jacobian(
        x=cen[1],
        y=cen[0],
        wcs=wcs.jacobian(image_pos=galsim.PositionD(cen[1], cen[0])),
    )
    psf_jacobian = ngmix.Jacobian(
        x=psf_cen[1],
        y=psf_cen[0],
        wcs=wcs.jacobian(image_pos=galsim.PositionD(psf_cen[1], psf_cen[0])),
    )
...
def test_metacal_accuracy(mcal_class, has_pixel):
    ...
    for _ in range(ntrial):
        ...
        for stype, mcal_obs in obs_dict.items():
            sres = boot.go(obs=mcal_obs)
            st = _make_struct(res=sres, obs=mcal_obs, shear_type=stype)
            dlist.append(st)
