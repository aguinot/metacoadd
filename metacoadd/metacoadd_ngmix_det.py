...
class MetaCoadd(SimpleCoadd):
    ...
    def go(self,):
        ...
        for type in self.types:
            non_zero_weights = \
                np.where(self.coaddimage.weight[type].array != 0)
            ...
            if 'noise' in list(all_stamp.keys()):
                self.coaddimage.noise[type].array[non_zero_weights] /= \
                    self.coaddimage.weight[type].array[non_zero_weights]
            self.psf_coaddimage.image[type].array[:, :] /= \
                len(self.psf_coaddimage.explist)
    ...
    def _process_psf(self, psfs):
        ...
        if isinstance(psfs, list):
            if all(isinstance(n, np.ndarray) for n in psfs):
                self._inv_psflist, self._sigma_psflist, self._flux_psflist = \
    ...
    def _init_invpsf_nparray(self, psfs):
        ...
        for n, psf in enumerate(psfs):
            ...
            sigma_psflist.append(self._get_exp_reconv_psf_size(
    ...
    def _add_positions_and_psf(self, mbobs, cat, res, shear_str):
        ...
        if cat.size > 0:
            ...
            if 'ormask_region' in TEST_METADETECT_CONFIG and TEST_METADETECT_CONFIG['ormask_region'] > 1:
                ormask_region = TEST_METADETECT_CONFIG['ormask_region']
            elif 'mask_region' in TEST_METADETECT_CONFIG and TEST_METADETECT_CONFIG['mask_region'] > 1:
                ormask_region = TEST_METADETECT_CONFIG['mask_region']
            else:
                ormask_region = 1

            if 'mask_region' in TEST_METADETECT_CONFIG and TEST_METADETECT_CONFIG['mask_region'] > 1:
