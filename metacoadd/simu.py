...
def run_metadetect(
    ...
    coadd_jac = ngmix.DiagonalJacobian(
        scale=simplecoadd.coaddimage.coadd_pixel_scale, row=cen, col=cen
    )
...
def run_metadetect_perfect(
    ...
    # Make ngmix Obs
    coadd_noise = rng.normal(size=coadd.image.array.shape) * np.sqrt(
        coadd.variance.array
    )
    obs = ngmix.Observation(
        image=coadd.image.array,
        weight=1.0 / coadd.variance.array,
        noise=coadd_noise,
        jacobian=coadd_jac,
        ormask=np.zeros_like(coadd.image.array, dtype=np.int32),
        bmask=np.zeros_like(coadd.image.array, dtype=np.int32),
        psf=ngmix.Observation(image=psf_image.array, jacobian=psf_jac),
    )
    mbobs = ngmix.MultiBandObsList()
    obslist = ngmix.ObsList()
    obslist.append(obs)
