...
import pyfftw
from math import sqrt, log, ceil, floor, exp
...
def shearmult(e1_a, e2_a, e1_b, e2_b):
    ...
    factor = (1.-sqrt(1-e1_b*e1_b-e2_b*e2_b)) / (e1_b*e1_b + e2_b*e2_b)
...
def bj_nullPSF(T_ratio, e1_gal, e2_gal, rho4_gal, e1_psf, e2_psf, rho4_psf):
    ...
    R = 1. - sig2ratio * (1+rho4_gal)/(1-rho4_gal) / cosheta_g \
        * (1-rho4_psf)/(1+rho4_psf)
...
def goodFFTSize(N):
    ...
    insize = (1.-1e-5)*N
    log2n = log(2.)*ceil(log(insize)/log(2.))
    log2n3 = log(3.) + log(2.)*ceil((log(insize)-log(3.))/log(2.))
    # must be even number
    log2n3 = max(log2n3, log(6.))
...
def get_resi_img(
    obs,
    xx_gal, yy_gal, xy_gal, flux_gal,
    xx_psf, yy_psf, xy_psf, flux_psf
):
    ...
    p_xmin = int(
        floor(
            obs.psf.jacobian.row0-nsig_rg2*sqrt(
                xx_gal/obs.jacobian.area
            ) - nsig_rg*fgauss_xsig
        )
    )
    p_xmax = int(
    ...
    p_ymin = int(
        floor(
            obs.psf.jacobian.col0-nsig_rg2*sqrt(
                yy_gal/obs.jacobian.area
            ) - nsig_rg*fgauss_ysig
        )
    )
    p_ymax = int(
...
def regauss(obs, psf_res, fitter=None, pars=None, do_fit=True, guess_fwhm=0.6):
    ...
    xx_final, yy_final, xy_final = get_corrected_mom3(
        e1_bj,
        e2_bj,
        T_gal*sqrt(1-(e1_gal**2+e2_gal**2))
    )
    ...
    return flux_gal, T_gal, Res, xx_final, yy_final, xy_final, 1.
